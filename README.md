...
不仅如此, 你还可以用 `waves()` 表达循环:
```cpp
waves({1, 9}, 4)                // w1~w9 每 4 波一循环
waves({1, 9}, 4, {11, 19}, 4)   // w1~w9, w11~w19 每4波一循环
```

 例如 `waves({1, 9}, 4)` 表示从 w1 开始, 到 w9 为止, 4 波一循环, 即在 w1, w5, w9 执行循环体中的内容.
...
> 对常用的 15 种植物, Simple AvZ 提供了(合理的)缩写.
...
就可以检验是否 w1~w20 每波正好调用一次. **强烈推荐**使用此功能, 可以免去不小心漏某波或重复某波等各种麻烦.
...
设定波次. 用法为 `for (auto w : waves(...)) { // 具体操作 }`.

```cpp
ValidateWaves();     // 检验 waves 完整性
```

检验是否 w1~w20 每波正好调用一次. 脚本编写完毕后, 加入此语句即可.
